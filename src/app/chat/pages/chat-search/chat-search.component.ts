...
export class ChatSearchComponent implements OnInit {
  viewModel$: Observable<ChatSearchViewModel> = this.store.select(selectChatSearchViewModel)
  ...
  headerActions$: Observable<Action[]> = this.viewModel$.pipe(
    map((vm) => {
      const actions: Action[] = [
        ...
        {
          labelKey: vm.chartVisible ? 'CHAT_SEARCH.HEADER_ACTIONS.HIDE_CHART' : 'CHAT_SEARCH.HEADER_ACTIONS.SHOW_CHART',
          icon: PrimeIcons.EYE,
          titleKey: vm.chartVisible ? 'CHAT_SEARCH.HEADER_ACTIONS.HIDE_CHART' : 'CHAT_SEARCH.HEADER_ACTIONS.SHOW_CHART',
          show: 'asOverflow',
  ...
  public chatSearchFormGroup: FormGroup = this.formBuilder.group({
    ...(Object.fromEntries(chatSearchCriteriasSchema.keyof().options.map((k) => [k, null])) as Record<
      keyof ChatSearchCriteria,
      unknown
    >)
  } satisfies Record<keyof ChatSearchCriteria, unknown>)

  type$ = enumToDropdownOptions(this.translateService, ChatType, 'CHAT_SEARCH.CRITERIA.TYPE.')
  ...
  ngOnInit() {
    ...
    this.viewModel$.subscribe((vm) => this.chatSearchFormGroup.patchValue(vm.searchCriteria))
  }
  ...
  search(formValue: FormGroup) {
    ...
    this.store.dispatch(ChatSearchActions.searchButtonClicked({ searchCriteria }))
  }
  ...
  onDisplayedColumnsChange(event: Event): void {
    ...
    this.store.dispatch(ChatSearchActions.displayedColumnsChanged({ displayedColumns }))
  }
